...
const SkillCard = ({ stack, className = '' }: SkillCardProps) => {
	return (
		...
		>
			...
			>
				<h3 className="font-semibold mb-6 text-2xl">{stack.label}</h3>

				<div className="flex flex-wrap items-center justify-center h-[calc(100%-56px)] gap-x-8 gap-y-4">
					{stack.skills?.map((skill: TechIconId) => {
						...
						return (
							<div
								key={skill}
								className="flex flex-col items-center min-h-[80px] gap-2 w-16 text-center"
							>
								{Icon ? (
									<Icon
										color={color}
